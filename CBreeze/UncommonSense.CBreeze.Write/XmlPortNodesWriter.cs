    ...
    public static class XmlPortNodesWriter
    {
        ...
        public static string IndentationAfterLastTrigger(IEnumerable<Property> relevantProperties)
        {
            ...
            if (lastProperty != null)
            {
                TypeSwitch.Do(
                    lastProperty,
                    TypeSwitch.Case<ScopedTriggerProperty>(p => result = p.Value.InvalidTrigger? String.Empty : new string(' ', p.ScopedName().Length + 2)),
                    TypeSwitch.Case<TriggerProperty>(p => result = p.Value.InvalidTrigger ? String.Empty : new string(' ', p.Name.Length + 2)));
            }
